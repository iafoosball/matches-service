    ...
    stages {
        stage ("Prepare stag environment") {
            steps {
                ...
                sh "docker stop \$5(docker ps -aqf name=matches-arangodb-stag) &"
                sh "docker rm \$5(docker ps -aqf name=matches-arangodb-stag) &"
                sh "docker stop \$5(docker ps -aqf name=matches-stag) &"
                sh "docker rm \$5(docker ps -aqf name=matches-stag) &"
            }
        }
        ...
        */
        ...
        stage ("Production") {
           steps{
                ...
                sh "docker stop \$5(docker ps -aqf name=matches-prod) &"
                sh "docker stop \$5(docker ps -aqf name=matches-arangodb-prod) &"
                sh "docker rm \$5(docker ps -aqf name=matches-arangodb-prod) &"
                sh "docker rm \$5(docker ps -aqf name=matches-prod) &"
                sh "sleep 15s"
